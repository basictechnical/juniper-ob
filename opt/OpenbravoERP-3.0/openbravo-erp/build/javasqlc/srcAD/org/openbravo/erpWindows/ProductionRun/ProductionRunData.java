 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET M_Production_ID = (?) , Productionplandate = TO_DATE(?) , Line = TO_NUMBER(?) , MA_Wrphase_ID = (?) , Starttime = TO_TIMESTAMP(?,'HH24:MI:SS') , Endtime = TO_TIMESTAMP(?,'HH24:MI:SS') , MA_Costcenter_Version_ID = (?) , Calccost = TO_NUMBER(?) , Neededquantity = TO_NUMBER(?) , ProductionQty = TO_NUMBER(?) , Secondaryunit = (?) , Conversionrate = TO_NUMBER(?) , Secondaryqty = TO_NUMBER(?) , Rejectedquantity = TO_NUMBER(?) , MA_Costcenteruse = TO_NUMBER(?) , Usedmaterial = (?) , Outsourced = (?) , Processed = (?) , Estimatedtime = TO_NUMBER(?) , Runtime = TO_NUMBER(?) , Closephase = (?) , Validating = (?) , AD_Client_ID = (?) , IsActive = (?) , M_Locator_ID = (?) , AD_Org_ID = (?) , Description = (?) , M_ProductionPlan_ID = (?) , M_Product_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (M_Production_ID, Productionplandate, Line, MA_Wrphase_ID, Starttime, Endtime, MA_Costcenter_Version_ID, Calccost, Neededquantity, ProductionQty, Secondaryunit, Conversionrate, Secondaryqty, Rejectedquantity, MA_Costcenteruse, Usedmaterial, Outsourced, Processed, Estimatedtime, Runtime, Closephase, Validating, AD_Client_ID, IsActive, M_Locator_ID, AD_Org_ID, Description, M_ProductionPlan_ID, M_Product_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), TO_DATE(?), TO_NUMBER(?), (?), TO_TIMESTAMP(?, 'HH24:MI:SS'), TO_TIMESTAMP(?, 'HH24:MI:SS'), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
